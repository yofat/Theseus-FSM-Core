...
def run_experiment_round(round_id):
    ...
    for i in range(config.MAX_TURNS):
        ...
        logs.append({
            ...
            "Victim_Prompt_Tokens(count)": victim_prompt_tokens,
            "Victim_Completion_Tokens(count)": victim_completion_tokens,
            "Attacker_Total_Prompt_Tokens(count)": attacker_stats['prompt_tokens'],
            "Attacker_Total_Completion_Tokens(count)": attacker_stats['completion_tokens'],
        })
...
if __name__ == "__main__":
    ...
    if args.out:
        ...
        results_dir = os.path.join(repo_root, "Keyword 1-1", "results")
